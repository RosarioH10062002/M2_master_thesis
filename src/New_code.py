...
FB_LIST = [10, 10, 10, 10, 10, 10]  
IMG_GO = "white_go_task.png"      
IMG_NOGO_FILES = [
...
def run_afc_block(its_ratio, fc=400, fb=10,
    ...
    for trial in range(n_trials):
        ...
        question_text.draw()
        win.flip()
        resp_key = event.waitKeys(keyList=['a', 'b', 'escape'])[0]

        if resp_key == 'escape':
            win.close()
            core.quit()

        if its_in_A and resp_key == 'a':
            correct_count += 1
            print(f"correct_count: {correct_count}")
        elif (not its_in_A) and resp_key == 'b':
            correct_count += 1
            print(f"correct_count: {correct_count}")
...
def run_gonogo_block(fb, its_ratio, fc=400,
    ...
    for k in range(N_TRIALS):
        ...
        trials_data.append({
            ...
            'fb': fb,
            'trial_index': k,
            'is_target': int(is_target),           # 1 = No-Go, 0 = Go
...
def run_full_paradigm(final_ratio, fc=400):
    all_trials = []
    ...
    # after 7 blocks, thank the participant
...
#----------------------------------------------------------------------------
#GET THE RESULTS 
def save_trials_to_csv(all_trials, filename="behavior_data.csv"):
    if not all_trials:
...
def main():
    # 2AFC 
    final_ratio = find_good_its_ratio(
        start_ratio=0.1, 
        step=0.02,
        max_ratio=0.4,
        target_perf=0.7
    ...
    #PARADIGM
    all_trials = run_full_paradigm(final_ratio, 
                                    fc=400)

    #SAVE RESULTS 
    save_trials_to_csv(all_trials, 
                        filename="subject01_behavior.csv")
